...
async function sendTicket(user_id, category, title, message, date){
    try {
        const response = await fetch('../api/tickets/sendTicket.php', {
            ...
            body: JSON.stringify({ user_id, category, title, message , date})
        });
...
async function createTicketForm(user_id){
    ...
    form.addEventListener("submit", async (e) => {
        ...
        await sendTicket(user_id, category, title, message, getToday());
