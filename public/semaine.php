...
<body>
    <?php
        ...
        $prix   = $_POST['prix']   ?? $_SESSION['prix']   ?? null;
        $sante  = $_POST['sante']  ?? $_SESSION['sante']  ?? null;
        $saison = $_POST['saison'] ?? $_SESSION['saison'] ?? null;

        if (!empty($_POST)) {
            $_SESSION['prix'] = $prix;
            $_SESSION['sante'] = $sante;
            $_SESSION['saison'] = $saison;
        }

$formulaire_remplie = ($prix !== null && $sante !== null && $saison !== null);
        ...
        function get_semaine_titre($conn, $semaine){
        ...
        function get_semaine_id($conn, $semaine){
            $semaine = $semaine[0] ?? null;
            //echo(count($semaine));
            $tableau_id_recettes = [];
            for ($i = 1; $i <= 14; $i++) {
        ...
        function compare_semaine_formulaire($conn, $semaine, $favoris_formulaire) {
            for ($i = 1; $i <= 14; $i++) {
                $id = $semaine["id_plat_$i"];
                if (!trouve_dans_favoris($id, $favoris_formulaire)) {
                    return FALSE;
                }
            }
            return TRUE;
        ...
        if (!$formulaire_remplie) {         // SI PAS DE FORMULAIRE
            ...
            if ($semaine){                  // SI PAS DE FORMULAIRE // MAIS UNE SEMAINE
                ...
                if (count($recettes) < 14){             // SI PAS DE FORMULAIRE // UNE SEMAINE // MAIS PAS ASSEZ DE FAVORIS
                    ...
                    // var_dump($semaine);
                    $tableau_nom_recettes = get_semaine_titre($conn, $semaine);
                    $tableau_id_recettes = get_semaine_id($conn, $semaine);
                    // var_dump($tableau_id_recettes);
    ...
    <div id='formulaire'>
        ...
        <form method="post" action="">
            ...
            <select name="prix">
                <option value="5" <?= (@$_SESSION["prix"] == 5) ? "selected" : "" ?>>5 (cher)</option>
                <option value="4" <?= (@$_SESSION["prix"] == 4) ? "selected" : "" ?>>4</option>
                <option value="3" <?= (@$_SESSION["prix"] == 3) ? "selected" : "" ?>>3</option>
                <option value="2" <?= (@$_SESSION["prix"] == 2) ? "selected" : "" ?>>2</option>
                <option value="1" <?= (@$_SESSION["prix"] == 1) ? "selected" : "" ?>>1 (abordable)</option>
            </select>

            <p>Indice de santé minimal:</p>
            <select name="sante">
                <option value="5" <?= (@$_SESSION["sante"] == 5) ? "selected" : "" ?>>5 (sain)</option>
                <option value="4" <?= (@$_SESSION["sante"] == 4) ? "selected" : "" ?>>4</option>
                <option value="3" <?= (@$_SESSION["sante"] == 3) ? "selected" : "" ?>>3</option>
                <option value="2" <?= (@$_SESSION["sante"] == 2) ? "selected" : "" ?>>2</option>
                <option value="1" <?= (@$_SESSION["sante"] == 1) ? "selected" : "" ?>>1 (mauvais)</option>
            </select>

            <p>Saison :</p>
            <select name="saison">
                <option value="printemps" <?= (@$_SESSION["saison"] == 'printemps') ? "selected" : "" ?>>printemps</option>
                <option value="ete" <?= (@$_SESSION["saison"] == 'ete') ? "selected" : "" ?>>été</option>
                <option value="automne" <?= (@$_SESSION["saison"] == 'automne') ? "selected" : "" ?>>automne</option>
                <option value="hiver" <?= (@$_SESSION["saison"] == 'hiver') ? "selected" : "" ?>>hiver</option>
                <option value="all" <?= (@$_SESSION["saison"] == 'all') ? "selected" : "" ?>>toutes saisons</option>
            </select>
